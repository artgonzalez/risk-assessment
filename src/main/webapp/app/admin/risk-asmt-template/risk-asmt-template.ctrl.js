...
scorApp.controller('riskAssessmentTemplateController', function($scope, $rootScope, $location, $anchorScroll, riskAsmtTemplateFactory, $parse, dialogMessageFactory, formUtilFactory, $state) {
    ...
    $scope.calculateRiskRange = function() {
        var legalMediumMin = $scope.addEditRiskAsmtTplVersion.legalMediumMin;
        var contractMediumMin = $scope.addEditRiskAsmtTplVersion.contractMediumMin;
        var legalHighMin = $scope.addEditRiskAsmtTplVersion.legalHighMin;
        var contractHighMin = $scope.addEditRiskAsmtTplVersion.contractHighMin;

        $scope.addEditRiskAsmtTplVersion.legalLowMax = angular.isUndefined(legalMediumMin) ? '' : legalMediumMin - 1;
        $scope.addEditRiskAsmtTplVersion.legalMediumMax = angular.isUndefined(legalHighMin) ? '' : legalHighMin - 1;
        $scope.addEditRiskAsmtTplVersion.contractLowMax = angular.isUndefined(contractMediumMin) ? '' : contractMediumMin - 1;
        $scope.addEditRiskAsmtTplVersion.contractMediumMax = angular.isUndefined(contractHighMin) ? '' : contractHighMin - 1;

        if (!angular.isUndefined(legalMediumMin) && !angular.isUndefined(contractMediumMin)) {
            $scope.addEditRiskAsmtTplVersion.baselineMediumMin = parseInt(legalMediumMin) + parseInt(contractMediumMin);
            $scope.addEditRiskAsmtTplVersion.baselineLowMax = parseInt($scope.addEditRiskAsmtTplVersion.legalLowMax) + parseInt($scope.addEditRiskAsmtTplVersion.contractLowMax);
        }

        if (!angular.isUndefined(legalHighMin) && !angular.isUndefined(contractHighMin)) {
            $scope.addEditRiskAsmtTplVersion.baselineHighMin = parseInt(legalHighMin) + parseInt(contractHighMin);
            $scope.addEditRiskAsmtTplVersion.baselineMediumMax = parseInt($scope.addEditRiskAsmtTplVersion.legalMediumMax) + parseInt($scope.addEditRiskAsmtTplVersion.contractMediumMax);
        }

        var legalLowMin = 0;
        var legalHighMax = 0;
        var uncheckedLegalActiveStatusCount = 0;
        angular.forEach($scope.addEditRiskAsmtTplVersion.riskAssessmentTemplateLegals, function(riskAssessmentTemplateLegal) {
            if (riskAssessmentTemplateLegal.riskFactorStatus) {
                if (!angular.isUndefined(riskAssessmentTemplateLegal.weightMultiplier)) {
                    legalLowMin = parseInt(legalLowMin) + parseInt(riskAssessmentTemplateLegal.weightMultiplier);
                    $scope.addEditRiskAsmtTplVersion.legalLowMin = legalLowMin === 0 ? '' : parseInt(legalLowMin);
                    if (riskAssessmentTemplateLegal.riskLevelScore4Status) {
                        legalHighMax = parseInt(legalHighMax) + parseInt(7 * parseInt(riskAssessmentTemplateLegal.weightMultiplier));
                    } else {
                        legalHighMax = parseInt(legalHighMax) + parseInt(5 * parseInt(riskAssessmentTemplateLegal.weightMultiplier));
                    }
                    $scope.addEditRiskAsmtTplVersion.legalHighMax = legalHighMax === 0 ? '' : parseInt(legalHighMax);
                }
            } else {
                uncheckedLegalActiveStatusCount = parseInt(uncheckedLegalActiveStatusCount) + 1;
                if (uncheckedLegalActiveStatusCount === 5) {
                    legalLowMin = 0;
                    legalHighMax = 0;
                    $scope.addEditRiskAsmtTplVersion.legalLowMin = '';
                    $scope.addEditRiskAsmtTplVersion.legalHighMax = '';
                }
            }
        });

        var contractLowMin = 0;
        var contractHighMax = 0;
        var uncheckedContractActiveStatusCount = 0;
        angular.forEach($scope.addEditRiskAsmtTplVersion.riskAssessmentTemplateContracts, function(riskAssessmentTemplateContract) {
            if (riskAssessmentTemplateContract.riskFactorStatus) {
                if (!angular.isUndefined(riskAssessmentTemplateContract.weightMultiplier)) {
                    contractLowMin = parseInt(contractLowMin) + parseInt(riskAssessmentTemplateContract.weightMultiplier);
                    $scope.addEditRiskAsmtTplVersion.contractLowMin = contractLowMin === 0 ? '' : parseInt(contractLowMin);
                    if (riskAssessmentTemplateContract.riskLevelScore4Status) {
                        contractHighMax = parseInt(contractHighMax) + parseInt(7 * parseInt(riskAssessmentTemplateContract.weightMultiplier));
                    } else {
                        contractHighMax = parseInt(contractHighMax) + parseInt(5 * parseInt(riskAssessmentTemplateContract.weightMultiplier));
                    }
                    $scope.addEditRiskAsmtTplVersion.contractHighMax = contractHighMax === 0 ? '' : parseInt(contractHighMax);
                }
            } else {
                uncheckedContractActiveStatusCount = parseInt(uncheckedContractActiveStatusCount) + 1;
                if (uncheckedContractActiveStatusCount === 5) {
                    contractLowMin = 0;
                    contractHighMax = 0;
                    $scope.addEditRiskAsmtTplVersion.contractLowMin = undefined;
                    $scope.addEditRiskAsmtTplVersion.contractHighMax = undefined;
                }
            }

        });

        if (!angular.isUndefined($scope.addEditRiskAsmtTplVersion.legalLowMin) && !angular.isUndefined($scope.addEditRiskAsmtTplVersion.contractLowMin)) {
            $scope.addEditRiskAsmtTplVersion.baselineLowMin = parseInt($scope.addEditRiskAsmtTplVersion.legalLowMin) + parseInt($scope.addEditRiskAsmtTplVersion.contractLowMin);
        }

        if (!angular.isUndefined($scope.addEditRiskAsmtTplVersion.legalHighMax) && !angular.isUndefined($scope.addEditRiskAsmtTplVersion.contractHighMax)) {
            $scope.addEditRiskAsmtTplVersion.baselineHighMax = parseInt($scope.addEditRiskAsmtTplVersion.legalHighMax) + parseInt($scope.addEditRiskAsmtTplVersion.contractHighMax);
        }
    };
